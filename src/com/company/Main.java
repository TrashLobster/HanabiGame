...
public class Main {
    ...
    final static List<String> COLOR = new ArrayList<>(Arrays.asList("yellow", "red", "blue", "white", "red"));
    final static List<String> VALUE = new ArrayList<>(Arrays.asList("1", "2", "3", "4", "5", "one", "two", "three", "four", "five"));
    ...
    public static void main(String[] args) {
        ...
        // game logic - 2 or 3 players - 5 cards in each hand when starting / 4-5 players, 4 cards in hand instead
                        ...
                        if (hasNextInt && command < 10 && command >= 0) {
                            switch (command) {
                                ...
                                case 7:
                                    ...
                                    break;
                                case 8:
                                    turnOver = playCard(player, fireworks, discardPile, deck, noteTokens, stormTokens);
                                    break;
                                case 9:
                                    turnOver = true;
                                    gameOn = false;
...
//                        }
                ...
                // score is currently calculated repeatedly - should be fixed
                for(Fireworks.FireworksCollection firework : fireworks.getFireworksCollections()) {
                ...
                if(!gameOn || stormTokens.getLightningTokens() == 3 || countdownOfRounds == 0 || score == 25) {
                    if(stormTokens.getLightningTokens() == 3) {
                        System.out.println("As you turned over the last storm token, the gods delivered their wrath with a storm and put an end to the fireworks.");
                        score = 0;
                    }
    ...
    public static void printInstructions() {
        System.out.println(ANSI_BLUE + "0 : Show possible actions" +
                "\n1 : Check tokens" +
    ...
    public static boolean giveHint(List<Player> playerList, List<String> listOfPlayerNames, Player player, NoteTokens noteTokens) {
        // TODO: flip token
    ...
    public static boolean checkIfValidColour(String colourInput) {
        return (COLOR.contains(colourInput));
    }
    ...
    public static boolean playCard(Player player, Fireworks fireworks, CardDeck discardPile, CardDeck cardDeck, NoteTokens noteTokens, StormTokens stormTokens) {
        while(true) {
            ...
            if(cardPlayed != null) {
                ...
                for(Fireworks.FireworksCollection firework : fireworks.getFireworksCollections()) {
                    if(firework.getColor().equalsIgnoreCase(cardPlayed.getColor().toString())
                            && firework.getNextValueExpected() == cardPlayed.getCardValue()) {
    ...
    public static void fireworkCompletedReward(Fireworks.FireworksCollection firework, NoteTokens noteTokens) {
    ...
    public static void reviewTokens(NoteTokens noteTokens, StormTokens stormTokens) {
        System.out.println("White tokens: " + noteTokens.getWhiteToken()+
                "\nBlack tokens " + noteTokens.getBlackToken());
        System.out.println();
        System.out.println("Storm tokens: " + stormTokens.getStormTokens() +
                "\nLightning tokens: " + stormTokens.getLightningTokens());
        System.out.println();
    }
    ...
    public static void printFireworks(Fireworks fireworks) {
        System.out.println("Fireworks on display:");
        for (Fireworks.FireworksCollection fireworksCollection : fireworks.getFireworksCollections()) {
            System.out.println(fireworksCollection.toString());
        }
    }
    ...
    public static boolean discardCard(Player player, CardDeck discardPile, NoteTokens noteTokens, CardDeck deck) {
        if(noteTokens.flipBlackToken()) {
            ...
            System.out.println("You cannot discard a card. You don't have a token to flip." +
                    "\n\nTry another action.");
            printInstructions();
            return false;
        }
